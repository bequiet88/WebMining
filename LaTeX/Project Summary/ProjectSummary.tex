...
A thorough analysis of alternatives lead us the way to GATE, the "Genereal Architecture for Text Extraction". \citeauthor*{Cunningham2002} \citeyearpar{Cunningham2002} states about their tool that "[t]he GATE architecture has enabled us not only to develop a number of successful applications for various  language  processing  tasks  (such as  Information  Extraction),  but  also to  build  and  annotate  corpora  and carry out evaluations on the applications generated." A more detailed look on GATE unveiled its powerful and extensive functions. By default, ... % describe default functions and JAPE grammer, JAPE example, xml export, parse xml for pos / neg sentences.


%the tool , abbreviated by GATE. 
